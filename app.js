...
const AGGIORNAMENTO_INTERVALLO = 60000; // 60 secondi per i prezzi intraday
const RETRY_INTERVALLO = 30000; // 30 secondi tra i tentativi di retry
const MAX_RETRY = 10; // Numero massimo di tentativi

// Cache per i prezzi (persistente in localStorage)
const CACHE_CHIAVE = "prezzi_etf_cache";

// ============================================
// CONFIGURAZIONE GRAFICO MULTI-LINEA
// ============================================
const COLORI_CRYPTO = '#f7931a'; // Arancione Bitcoin
const COLORI_ETF = '#2563eb'; // Blu
const COLORI_ETN = '#8b5cf6'; // Viola
const COLORI_AZIONI = '#22c55e'; // Verde
...
async function fetchTickerDataConRetry(ticker, range = "1d", interval = "1m", tentativi = 0) {
    ...
    // Controlla la cache persistente (valida 1 ora per i prezzi intraday)
    ...
    try {
        ...
        if (data && data.chart && data.chart.result && data.chart.result.length > 0) {
            ...
            // Verifica che il prezzo sia in EUR (o convertito)
            if (currency !== "EUR" && currency !== "USD (non convertito)") {
                console.log(`⚠️ Attenzione: valuta ${currency} per ${ticker}`);
            }
            
            // Per Bitcoin ETN, verifica che il prezzo sia tra 5€ e 30€
            ...
            // Per gli altri ticker, salva in cache
            cachePrezzi[cacheKey] = {
                data: result,
                timestamp: now
            };
...
// ============================================
// RECUPERO DATI ASSET (CON RETRY PER BITCOIN)
// ============================================
async function recuperaDatiAsset(item) {
    try {
        // PER L'ETN BITCOIN - Usa retry con cache persistente
...
function isAssetCrypto(item) {
    ...
    // Escludiamo esplicitamente l'ETN Bitcoin (GB00BJYDH287) dal conteggio crypto
...
// ============================================
// FUNZIONI GRAFICO (VERSIONE MULTI-LINEA CON DOPPIO ASSE Y)
// ============================================
...
async function aggiornaGrafico() {
    ...
    // Se è selezionato un asset specifico
    ...
    // ============================================
    // GRAFICO MULTI-LINEA: PORTAFOGLIO TOTALE
    // ============================================
    ...
    // Prendi il primo asset come riferimento per le date
    ...
    // Prepara i datasets per ogni categoria
    const datasets = [];
    
    // 1. DATASET CRYPTO (solo crypto vere, escluso ETN)
    ...
    // 2. DATASET ETF/ETC + ETN BITCOIN (TUTTO INSIEME)
    const assetETF = assetAcquistati.filter(item => {
        // Includiamo ETF, ETC e l'ETN Bitcoin
    ...
    // Ordina i datasets: prima Crypto, poi ETF/ETC/ETN
...
function renderizzaGraficoMulti(labels, datasets, titolo) {
    ...
    // Prepara i datasets per Chart.js con due assi Y
    const chartDatasets = datasets.map((ds) => {
        // Determina quale asse usare in base al label
        let yAxisID = 'y';
        let borderDash = [];
        
        if (ds.label.includes('Crypto')) {
            yAxisID = 'y'; // Asse sinistro per Crypto
            borderDash = [];
        } else if (ds.label.includes('ETF') || ds.label.includes('ETN')) {
            yAxisID = 'y1'; // Asse destro per ETF/ETN
    ...
    // Configurazione del grafico con due assi Y
    graficoIstanza = new Chart(ctx, {
        type: 'line',
        data: {
            labels: labels,
...
// Aggiornamento ogni 30 secondi per i prezzi intraday
setInterval(aggiornaTutto, AGGIORNAMENTO_INTERVALLO);

// Ogni ora, controlla se il prezzo Bitcoin è stato aggiornato oggi
setInterval(() => {
    const now = new Date();
    const oggi = now.toDateString();
    const cache = caricaCache();
    
    // Controlla se abbiamo un prezzo Bitcoin valido di oggi
    const cacheKey = "BTCW.MI_1d_1m";
    if (cache[cacheKey]) {
        const dataCache = new Date(cache[cacheKey].timestamp);
        const dataCacheStr = dataCache.toDateString();
        
        if (dataCacheStr !== oggi) {
            console.log("📅 Nuovo giorno, resetto cache Bitcoin...");
            delete cache[cacheKey];
            salvaCache(cache);
            prezzoBitcoinValido = false;
            aggiornaTutto();
        }
    }
}, 3600000); // Ogni ora

console.log("🚀 App avviata - Aggiornamento ogni 30 secondi");
console.log("📦 Cache persistente attiva");
