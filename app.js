...
const AGGIORNAMENTO_INTERVALLO = 60000; // 60 secondi (meno chiamate su mobile)
    ...
    switch(stato) {
        case 'synced':
            ...
            if (btnSync) {
                ...
                btnSync.innerText = '☁️ Sincronizzato';
            }
            break;
        case 'syncing':
...
async function salvaDatiCloud(dati) {
    ...
    try {
        ...
        const response = await fetch(`${WORKER_URL}/save`, {
            ...
            body: JSON.stringify(dati || portafoglio)
        });
        
        if (response.ok) {
            const result = await response.json();
            console.log("✅ Dati salvati su cloud:", result);
            aggiornaStatoSincronizzazione('synced', 'Salvato');
            return true;
        } else {
            console.error("❌ Errore salvataggio cloud:", response.status);
            aggiornaStatoSincronizzazione('error', 'Errore salvataggio');
            return false;
        }
    } catch (e) {
        console.error("❌ Errore salvataggio cloud:", e);
        aggiornaStatoSincronizzazione('error', 'Errore di rete');
        return false;
    } finally {
...
async function sincronizzaOra() {
    ...
    if (datiLocali.length > 0) {
        ...
        if (datiCloud && datiCloud.length > 0) {
            ...
            aggiornaTutto();
            aggiornaStatoSincronizzazione('synced', 'Dati ripristinati');
        }
    }
}

// ============================================
// INIZIALIZZAZIONE PORTAFOGLIO (con sincronizzazione)
// ============================================
async function inizializzaPortafoglio() {
    ...
    if (datiCloud && datiCloud.length > 0) {
        ...
        aggiornaStatoSincronizzazione('synced', `Caricati ${datiCloud.length} asset`);
    } else {
        // Usa i dati locali o default
        ...
        // Salva i dati locali sul cloud (se non esistono)
    ...
    // 3. Sincronizza periodicamente (ogni 5 minuti)
    setInterval(async () => {
