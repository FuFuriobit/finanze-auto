...
const CACHE_TTL_FRESH = 5; // minuti
const CACHE_TTL_STALE = IS_MOBILE ? 30 : 15; // minuti (mostra subito prezzi in cache)
const FETCH_CONCURRENCY = IS_MOBILE ? 2 : 5;
const CHART_DEFER_MS = IS_MOBILE ? 2500 : 800;
...
// ============================================
// PRECARICAMENTO DA CACHE (per velocità)
// ============================================
...
function renderTabellaPortafoglio() {
    // Mostra subito i dati anche se i prezzi non sono ancora caricati
    let rowsHtml = "";
    for (let i = 0; i < portafoglio.length; i++) {
        ...
        rowsHtml += `
            <tr>
                ...
                <td>0,00 €</td>
                <td><strong>${!hasError ? (0).toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' €' : 'N/D'}</strong></td>
                <td class="neg">N/D</td>
                <td class="neg">N/D</td>
                <td>
...
async function fetchTickerDataConRetry(ticker, range = "1d", interval = "1m", tentativi = 0) {
    ...
    // Cache: usa sempre se disponibile (anche se vecchia) per velocità
    if (cachePrezzi[cacheKey]) {
        const cached = cachePrezzi[cacheKey];
        const eta = now - cached.timestamp;
        const minuti = eta / (1000 * 60);
        
        // Su mobile: usa cache anche se più vecchia (fino a 30 minuti)
    ...
    try {
        ...
        const response = await fetch(url, {
            signal: AbortSignal.timeout(FETCH_TIMEOUT || 10000)
        });
        
        // 🔥 SE 404 → NON RITENTARE!
        if (response.status === 404) {
            console.warn(`⚠️ Ticker ${ticker} non trovato (404). Salto definitivo.`);
            return null;
        }
        ...
        if (data && data.chart && data.chart.result && data.chart.result.length > 0) {
            ...
            // Salva in cache anche se il prezzo è 0 (evita ritentativi)
            ...
            if (prezzo > 0) {
                console.log(`✅ Prezzo ${ticker}: ${prezzo} EUR`);
            } else {
                console.log(`⚠️ Prezzo ${ticker}: 0 (dato non disponibile)`);
            }
            return result;
        }
    } catch (e) {
...
async function recuperaDatiAsset(item) {
    try {
        ...
        if (item.isin === "GB00BJYDH287") {
            ...
            // 🔥 Se non c'è prezzo, prova a usare la cache più vecchia
            ...
            if (cachePrezzi[cacheKey]) {
                ...
                let prezzoAttuale = meta.regularMarketPrice || meta.previousClose || 0;
                if (prezzoAttuale > 0) {
                    console.log(`📦 Fallback cache per BTCW.MI (dato vecchio)`);
                    return {
                        prezzo: parseFloat(prezzoAttuale),
                        varGiornaliera: 0
                    };
        ...
        // 🔥 Fallback alla cache per qualsiasi ticker
        const cacheKey = `${item.ticker}_1d_1m`;
        if (cachePrezzi[cacheKey]) {
            const cached = cachePrezzi[cacheKey];
            const meta = cached.data.meta;
            let prezzoAttuale = meta.regularMarketPrice || meta.previousClose || 0;
            if (prezzoAttuale > 0) {
                console.log(`📦 Fallback cache per ${item.ticker} (dato vecchio)`);
                return {
                    prezzo: parseFloat(prezzoAttuale),
                    varGiornaliera: 0
                };
...
async function aggiornaTutto() {
    ...
    try {
        // Recupera i prezzi per tutti gli asset
